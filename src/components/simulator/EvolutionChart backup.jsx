...
export default function EvolutionChart({ data, filters, premiumPaid }) {
  ...
  const generateHeatmapData = () => {
    ...
    let expirationDate;
    if (selectedContract && simulationMode === 'ticker') {
      const dateStr = selectedContract.split(' - ')[1];
      expirationDate = dateStr;
    } else {
      const today = new Date();
      const expDate = new Date(today);
      expDate.setDate(expDate.getDate() + daysToExpiration);
      expirationDate = expDate.toISOString().split('T')[0];
    }

    const heatmapDates = getHeatmapDates(expirationDate, daysToExpiration);
