...
/**
 * Cboe's own implied-volatility indices, keyed by the underlying they price.
 *
 * These are the only *implied* volatility history available from this feed: the
 * delayed-quotes chain serves today's IV and nothing older, but each of these
 * indices is published as a daily series going back a decade or more, exactly
 * like a price history. Where a ticker is on this list its implied volatility
 * can be ranked against its own past for real; everywhere else it has to be
 * estimated (see `impliedVolProxySeries` in src/lib/volatilityHistory.js).
 *
 * Each index measures 30-day at-the-money-ish implied volatility of its
 * underlying, computed the same way VIX is computed for the S&P 500.
 */
export const VOLATILITY_INDICES = {
  SPY: { symbol: '_VIX', name: 'VIX', of: 'the S&P 500' },
  SPX: { symbol: '_VIX', name: 'VIX', of: 'the S&P 500' },
  QQQ: { symbol: '_VXN', name: 'VXN', of: 'the Nasdaq 100' },
  NDX: { symbol: '_VXN', name: 'VXN', of: 'the Nasdaq 100' },
  IWM: { symbol: '_RVX', name: 'RVX', of: 'the Russell 2000' },
  RUT: { symbol: '_RVX', name: 'RVX', of: 'the Russell 2000' },
  DIA: { symbol: '_VXD', name: 'VXD', of: 'the Dow' },
  DJX: { symbol: '_VXD', name: 'VXD', of: 'the Dow' },
  AAPL: { symbol: '_VXAPL', name: 'VXAPL', of: 'Apple' },
  AMZN: { symbol: '_VXAZN', name: 'VXAZN', of: 'Amazon' },
  GOOG: { symbol: '_VXGOG', name: 'VXGOG', of: 'Alphabet' },
  GOOGL: { symbol: '_VXGOG', name: 'VXGOG', of: 'Alphabet' },
  IBM: { symbol: '_VXIBM', name: 'VXIBM', of: 'IBM' },
  GS: { symbol: '_VXGS', name: 'VXGS', of: 'Goldman Sachs' },
  EEM: { symbol: '_VXEEM', name: 'VXEEM', of: 'emerging markets' },
  EWZ: { symbol: '_VXEWZ', name: 'VXEWZ', of: 'Brazil' },
  FXI: { symbol: '_VXFXI', name: 'VXFXI', of: 'China large caps' },
  GDX: { symbol: '_VXGDX', name: 'VXGDX', of: 'gold miners' },
  SLV: { symbol: '_VXSLV', name: 'VXSLV', of: 'silver' },
  TLT: { symbol: '_VXTLT', name: 'VXTLT', of: 'long Treasuries' },
  GLD: { symbol: '_GVZ', name: 'GVZ', of: 'gold' },
  USO: { symbol: '_OVX', name: 'OVX', of: 'crude oil' },
};

/** The Cboe volatility index for a ticker, or null when none is published. */
export function volatilityIndexFor(rawSymbol) {
  return VOLATILITY_INDICES[normalizeSymbol(rawSymbol)] || null;
}

/**
 * Daily closes of a ticker's own implied-volatility index, or null when Cboe
 * publishes none for it. Resolves rather than throws on a network failure: this
 * is one panel of the insights page, not the page itself.
 */
export async function fetchVolatilityIndexHistory(rawSymbol, { force = false } = {}) {
  const index = volatilityIndexFor(rawSymbol);
  if (!index) return null;

  try {
    const history = await fetchPriceHistory(index.symbol, { force });
    return history.length ? { ...index, history } : null;
  } catch {
    return null;
  }
}
